...
const PIcon = ({ pid, sz = 32 }) => {
  ...
  if (!p) return null;
  return (
    <div style={{ width: sz, height: sz, borderRadius: "50%", background: p.color, color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: sz * 0.38, fontWeight: "bold", flexShrink: 0, boxShadow: `0 2px 8px ${p.color}55` }}>
      {p.icon}
    </div>
  );
};
...
const Toggle = ({ on, onChange, color = "#6366F1" }) => (
...
const Field = ({ label, hint, children }) => (
  <div style={{ marginBottom: 14 }}>
    <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#374151", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.5 }}>{label}</label>
    {children}
...
export default function App() {
  ...
  const [settingsTab, setSTab] = useState("brand");
  ...
  const [apiKeys, setApiKeys] = useState(
    Object.fromEntries(PLATFORMS.map(p => [p.id, Object.fromEntries(p.apiFields.map(f => [f.key, ""]))]))
  );
  const [enabled, setEnabled] = useState(Object.fromEntries(PLATFORMS.map(p => [p.id, false])));
  ...
  const brandBlock = () => !brand.companyName ? "" : `BRAND CONTEXT:
...
All content MUST reflect this brand identity.\n`;
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          max_tokens: 1024,
          messages: [{ role: "user", content: `${brandBlock()}You are a social media copywriter. Create a ${form.type} for ${p.name} about "${form.topic}". Tone: ${form.tone}. Return ONLY valid JSON (no markdown): {"content":"post text with emojis","hashtags":"#tag1 #tag2 #tag3 #tag4 #tag5"}` }]
        }),
      });
      ...
      notify("Generation failed. Check your API key in Vercel.", "err");
    }
    setGen(false);
  };
  ...
  const brandOk = brand.companyName && brand.description && brand.targetAudience;
  ...
  return (
    <div style={{ fontFamily: "'Inter',system-ui,sans-serif", display: "flex", minHeight: "100vh", background: "#F8FAFC" }}>
      <div style={{ width: 210, background: "linear-gradient(160deg,#0F172A,#1E293B)", color: "white", display: "flex", flexDirection: "column", flexShrink: 0, position: "sticky", top: 0, height: "100vh" }}>
        ...
        <nav style={{ padding: "4px 10px", flex: 1 }}>
          {nav.map(item => (
            <button key={item.id} onClick={() => setTab(item.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 9, padding: "10px 12px", borderRadius: 8, border: "none", cursor: "pointer", background: tab === item.id ? "rgba(99,102,241,0.2)" : "transparent", color: tab === item.id ? "#A5B4FC" : "#94A3B8", fontSize: 13, fontWeight: tab === item.id ? 700 : 400, marginBottom: 2, textAlign: "left", borderLeft: tab === item.id ? "3px solid #6366F1" : "3px solid transparent" }}>
              ...
              {item.id === "settings" && !brandOk && <span style={{ marginLeft: "auto", width: 7, height: 7, borderRadius: "50%", background: "#F59E0B", flexShrink: 0 }} />}
            </button>
          ))}
        </nav>
        <div style={{ padding: 14, borderTop: "1px solid #1E293B" }}>
          ...
          {PLATFORMS.map(p => (
            <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 7, padding: "4px 0" }}>
              <div style={{ width: 7, height: 7, borderRadius: "50%", background: enabled[p.id] ? p.color : "#334155" }} />
              <span style={{ fontSize: 12, color: enabled[p.id] ? "#E2E8F0" : "#64748B", flex: 1 }}>{p.name.split(" ")[0]}</span>
              <span style={{ fontSize: 10, color: enabled[p.id] ? "#4ADE80" : "#475569" }}>{enabled[p.id] ? "●" : "○"}</span>
            </div>
          ))}
          <div style={{ marginTop: 8, fontSize: 11, color: "#475569" }}>{connectedCount}/5 connected</div>
        </div>
      </div>

      <div style={{ flex: 1, overflow: "auto", padding: "28px 32px" }}>
        {notif && (
          <div style={{ position: "fixed", top: 20, right: 20, zIndex: 9999, background: notif.t === "err" ? "#FEF2F2" : "#F0FDF4", border: `1px solid ${notif.t === "err" ? "#FECACA" : "#BBF7D0"}`, color: notif.t === "err" ? "#DC2626" : "#16A34A", padding: "12px 20px", borderRadius: 10, fontWeight: 700, fontSize: 14, boxShadow: "0 8px 24px rgba(0,0,0,0.12)" }}>
            {notif.msg}
          </div>
        )}

        {!brandOk && tab !== "settings" && (
          <div style={{ background: "linear-gradient(135deg,#FFF7ED,#FFFBEB)", border: "1.5px solid #FED7AA", borderRadius: 12, padding: "14px 18px", marginBottom: 20, display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "#B45309", marginTop: 2 }}>Add your company details so generated posts match your brand voice.</div>
            </div>
                ...
                {brandOk ? (
                  <div>
                    <div style={{ background: "linear-gradient(135deg,#EEF2FF,#F0FDF4)", borderRadius: 10, padding: "14px 16px", marginBottom: 12 }}>
                      <div style={{ fontSize: 17, fontWeight: 800, color: "#0F172A" }}>{brand.companyName}</div>
                  ...
                  <div style={{ textAlign: "center", padding: "20px 0" }}>
                    ...
                    <div style={{ fontSize: 13, color: "#64748B", marginBottom: 12 }}>Set up your brand profile so AI writes content that sounds like you</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  return (
                    <div style={{ background: "white", borderRadius: 12, padding: 16, border: `2px solid ${p.color}33` }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 9 }}>
                        <div style={{ width: 32, height: 32, borderRadius: "50%", background: brand.primaryColor || p.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>{brand.logoEmoji || "🏢"}</div>
                ...
                <div style={{ display: "flex", gap: 9 }}>
                  ...
                  <button onClick={schedulePost} disabled={saving} style={{ flex: 2, padding: "12px", borderRadius: 10, border: "none", cursor: saving ? "wait" : "pointer", background: saving ? "#86EFAC" : "linear-gradient(135deg,#10B981,#059669)", color: "white", fontSize: 13, fontWeight: 700 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 4, marginBottom: 22, background: "white", padding: 4, borderRadius: 12, border: "1px solid #E2E8F0", width: "fit-content" }}>
              {[{ id: "brand", l: "🎨 Brand Profile" }, { id: "apis", l: "🔑 API Keys" }].map(t => (
                <button key={t.id} onClick={() => setSTab(t.id)} style={{ padding: "9px 20px", borderRadius: 9, border: "none", cursor: "pointer", background: settingsTab === t.id ? "#6366F1" : "transparent", color: settingsTab === t.id ? "white" : "#64748B", fontWeight: settingsTab === t.id ? 700 : 500, fontSize: 13 }}>{t.l}</button>
              ))}
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                <div style={card}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "60px 1fr", gap: 12, marginBottom: 14, alignItems: "end" }}>
                    <Field label="Logo"><input value={brand.logoEmoji} onChange={e => setBrand(b => ({ ...b, logoEmoji: e.target.value }))} style={{ ...inp, textAlign: "center", fontSize: 22 }} maxLength={2} /></Field>
                    <Field label="Company Name *"><input value={brand.companyName} onChange={e => setBrand(b => ({ ...b, companyName: e.target.value }))} placeholder="Your company name" style={inp} /></Field>
                  </div>
                  <Field label="Tagline"><input value={brand.tagline} onChange={e => setBrand(b => ({ ...b, tagline: e.target.value }))} placeholder="Your catchy tagline" style={inp} /></Field>
                  <Field label="Industry"><select value={brand.industry} onChange={e => setBrand(b => ({ ...b, industry: e.target.value }))} style={{ ...inp, background: "white" }}>{INDUSTRIES.map(i => <option key={i}>{i}</option>)}</select></Field>
                  <Field label="Description *"><textarea value={brand.description} onChange={e => setBrand(b => ({ ...b, description: e.target.value }))} placeholder="What do you do? What makes you unique?" style={{ ...inp, minHeight: 80, resize: "vertical", lineHeight: 1.5 }} /></Field>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                  <div style={card}>
                    ...
                    <Field label="Target Audience *"><textarea value={brand.targetAudience} onChange={e => setBrand(b => ({ ...b, targetAudience: e.target.value }))} placeholder="Who are you speaking to?" style={{ ...inp, minHeight: 70, resize: "vertical", lineHeight: 1.5 }} /></Field>
                    <Field label="Brand Voice"><input value={brand.brandVoice} onChange={e => setBrand(b => ({ ...b, brandVoice: e.target.value }))} placeholder="Friendly, empowering, science-backed..." style={inp} /></Field>
                    <Field label="Key Messages"><textarea value={brand.keyMessages} onChange={e => setBrand(b => ({ ...b, keyMessages: e.target.value }))} placeholder="Core things you always want to say..." style={{ ...inp, minHeight: 60, resize: "vertical", lineHeight: 1.5 }} /></Field>
                    <Field label="Content Pillars"><input value={brand.contentPillars} onChange={e => setBrand(b => ({ ...b, contentPillars: e.target.value }))} placeholder="Education, Inspiration, Promotion..." style={inp} /></Field>
                    <Field label="Posting Goal"><input value={brand.postingGoal} onChange={e => setBrand(b => ({ ...b, postingGoal: e.target.value }))} placeholder="Drive traffic and grow community" style={inp} /></Field>
                    <Field label="Words to AVOID"><input value={brand.avoidWords} onChange={e => setBrand(b => ({ ...b, avoidWords: e.target.value }))} placeholder="toxic, cheap, competitor names..." style={inp} /></Field>
                  </div>
                  <button onClick={() => notify("✅ Brand profile saved!")} style={{ padding: "13px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#6366F1,#8B5CF6)", color: "white", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>💾 Save Brand Profile</button>
                  {brand.companyName && (
                    <div style={{ background: "linear-gradient(135deg,#EEF2FF,#F5F3FF)", borderRadius: 12, padding: 16, border: "1px solid #C7D2FE" }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#6366F1", marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>Live Preview</div>
                      ...
                      <div style={{ fontSize: 12, color: "#4338CA" }}>Industry: {brand.industry}</div>
                      {brand.brandVoice && <div style={{ fontSize: 12, color: "#4338CA" }}>Voice: {brand.brandVoice}</div>}
                    </div>
                  )}
                </div>
              </div>
            ...
            {settingsTab === "apis" && (
              <div>
                <div style={{ background: "#FFF7ED", border: "1.5px solid #FED7AA", borderRadius: 12, padding: "14px 18px", marginBottom: 20, display: "flex", gap: 12 }}>
                  <span style={{ fontSize: 20 }}>⚠️</span>
                  <div>
                    <div style={{ fontWeight: 700, color: "#92400E", fontSize: 14 }}>Keys stored in memory only</div>
                    <div style={{ fontSize: 12, color: "#B45309", marginTop: 3 }}>Your ANTHROPIC_API_KEY must be set as an Environment Variable in Vercel — not here. Social platform keys are entered here for session use only.</div>
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                  {PLATFORMS.map(p => (
                    <div key={p.id} style={{ ...card, borderLeft: `4px solid ${enabled[p.id] ? p.color : "#E2E8F0"}` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: enabled[p.id] ? 16 : 0 }}>
                        <PIcon pid={p.id} sz={36} />
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 700, fontSize: 15, color: "#0F172A" }}>{p.name}</div>
                          <div style={{ fontSize: 12, color: enabled[p.id] ? "#16A34A" : "#94A3B8" }}>{enabled[p.id] ? "● Connected" : "○ Not connected"}</div>
                        </div>
                        <Toggle on={enabled[p.id]} onChange={() => setEnabled(s => ({ ...s, [p.id]: !s[p.id] }))} color={p.color} />
                      </div>
                      {enabled[p.id] && (
                        <div>
                          {p.apiFields.map(field => (
                            <Field key={field.key} label={field.label}>
                              <div style={{ position: "relative" }}>
                                <input type={showKey[`${p.id}_${field.key}`] ? "text" : "password"} value={apiKeys[p.id][field.key]} onChange={e => setApiKeys(k => ({ ...k, [p.id]: { ...k[p.id], [field.key]: e.target.value } }))} placeholder={`Enter ${p.name} ${field.label}`} style={{ ...inp, paddingRight: 40 }} />
                                <button onClick={() => setShowKey(s => ({ ...s, [`${p.id}_${field.key}`]: !s[`${p.id}_${field.key}`] }))} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", fontSize: 14, color: "#94A3B8" }}>
                                  {showKey[`${p.id}_${field.key}`] ? "🙈" : "👁"}
                                </button>
                              </div>
                            </Field>
                          ))}
                          <button onClick={() => { const has = p.apiFields.every(f => apiKeys[p.id][f.key].trim()); notify(has ? `✅ ${p.name} ready!` : `Fill in all ${p.name} fields.`, has ? "ok" : "err"); }} style={{ width: "100%", padding: "9px", borderRadius: 8, border: "none", background: p.bg, color: p.color, fontWeight: 700, cursor: "pointer", fontSize: 13 }}>Test Connection</button>
                        </div>
                      )}
                    </div>
                  ))}
