...
\f0\fs24 \cf0 const res = await fetch("/api/generate", \{\
...
const PLATFORMS = [\
  ...
  \{ id: "instagram", name: "Instagram",  color: "#E1306C", bg: "#FCE4EC", icon: "\uc0\u55357 \u56567 ", apiFields: [\{ key: "accountId", label: "Business Account ID" \}, \{ key: "accessToken", label: "Access Token" \}] \},\
  ...
  \{ id: "tiktok",    name: "TikTok",     color: "#EE1D52", bg: "#FFF0F3", icon: "\uc0\u9834 ",  apiFields: [\{ key: "openId", label: "Open ID" \}, \{ key: "accessToken", label: "Access Token" \}] \},\
];\
\
...
\
  const [brand, setBrand] = useState(\{\
    ...
    brandVoice: "", avoidWords: "", primaryColor: "#6366F1", logoEmoji: "\uc0\u55356 \u57314 ",\
...
\
  const [posts, setPosts] = useState([\
    \{ id: 1, pid: "instagram", content: "\uc0\u55356 \u57119  New collection just dropped! Swipe to see the magic. Drop a \u10084 \u65039  if you love it!", topic: "Product Launch", tone: "Promotional", date: "2026-03-05 10:00", status: "scheduled", type: "Post" \},\
    ...
    \{ id: 3, pid: "tiktok",    content: "POV: You discovered the most aesthetic caf\'e9 in town \uc0\u9749  #CoffeeAesthetic #FYP", topic: "Coffee Shop", tone: "Casual", date: "2026-03-04 18:00", status: "posted", type: "Reel/Short" \},\
    \{ id: 4, pid: "threads",   content: "Hot take: The best productivity hack is actually taking more breaks. Change my mind. \uc0\u55357 \u56384 ", topic: "Productivity", tone: "Casual", date: "2026-03-04 14:00", status: "posted", type: "Post" \},\
    \{ id: 5, pid: "google",    content: "Grand Opening this weekend! Free samples, exclusive discounts, live demos 10AM\'966PM.", topic: "Grand Opening", tone: "Professional", date: "2026-03-06 09:00", status: "draft", type: "Announcement" \},\
  ]);\
\
...
- Company: $\{brand.companyName\}$\{brand.tagline ? ` \'97 "$\{brand.tagline\}"` : ""\}\
...
\
  const generate = async () => \{\
    ...
    try \{\
      ...
      notify("\uc0\u10024  Brand-aligned content generated!");\
...
\
  const schedule = () => \{\
    ...
    setTimeout(() => \{\
      ...
      notify("\uc0\u55357 \u56960  Post scheduled!"); setSaving(false); setTab("queue");\
    \}, 800);\
  \};\
\
...
\
  const navItems = [\
    \{ id: "dashboard", l: "Dashboard",   i: "\uc0\u9638 " \},\
    \{ id: "create",    l: "Create Post", i: "\uc0\u9999 " \},\
    \{ id: "queue",     l: "Queue",       i: "\uc0\u9776 " \},\
    \{ id: "calendar",  l: "Calendar",    i: "\uc0\u9707 " \},\
    \{ id: "settings",  l: "Settings",    i: "\uc0\u9881 " \},\
  ];\
\
  return (\
        ...
        <div style=\{\{ padding: 14, borderTop: "1px solid #1E293B" \}\}>\
          ...
          \{PLATFORMS.map(p => (\
            <div key=\{p.id\} style=\{\{ display: "flex", alignItems: "center", gap: 7, padding: "4px 0" \}\}>\
              ...
              <span style=\{\{ fontSize: 10, color: platformEnabled[p.id] ? "#4ADE80" : "#475569" \}\}>\{platformEnabled[p.id] ? "\uc0\u9679 " : "\u9675 "\}</span>\
            </div>\
          ))\}\
...
\
        \{!brandComplete && tab !== "settings" && (\
          <div style=\{\{ background: "linear-gradient(135deg,#FFF7ED,#FFFBEB)", border: "1.5px solid #FED7AA", borderRadius: 12, padding: "14px 18px", marginBottom: 20, display: "flex", alignItems: "center", gap: 14 \}\}>\
            <span style=\{\{ fontSize: 22 \}\}>\uc0\u55356 \u57256 </span>\
            <div style=\{\{ flex: 1 \}\}>\
            ...
            <button onClick=\{() => setTab("settings")\} style=\{\{ padding: "8px 16px", borderRadius: 8, border: "none", background: "#F59E0B", color: "white", fontWeight: 700, cursor: "pointer", fontSize: 13, whiteSpace: "nowrap" \}\}>Setup Brand \uc0\u8594 </button>\
          </div>\
        )\}\
\
        \{/* DASHBOARD */\}\
        \{tab === "dashboard" && (\
          <div>\
            ...
            <div style=\{\{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 20 \}\}>\
              \{[\{ l: "Total Posts", v: posts.length, i: "\uc0\u55357 \u56541 ", c: "#6366F1" \}, \{ l: "Scheduled", v: posts.filter(x => x.status === "scheduled").length, i: "\u9200 ", c: "#F59E0B" \}, \{ l: "Posted", v: posts.filter(x => x.status === "posted").length, i: "\u9989 ", c: "#10B981" \}, \{ l: "Drafts", v: posts.filter(x => x.status === "draft").length, i: "\u55357 \u56516 ", c: "#64748B" \}].map((s, i) => (\
            ...
            <div style=\{\{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginBottom: 18 \}\}>\
              ...
              <div style=\{card\}>\
                <h3 style=\{\{ margin: "0 0 14px", fontSize: 15, fontWeight: 700, color: "#0F172A" \}\}>\{brand.companyName ? `$\{brand.logoEmoji\} Brand Profile` : "\uc0\u55356 \u57256  Brand Profile"\}</h3>\
                \{brandComplete ? (\
                  <div>\
                    ...
                    <button onClick=\{() => setTab("settings")\} style=\{\{ marginTop: 8, fontSize: 12, color: "#6366F1", background: "none", border: "none", cursor: "pointer", fontWeight: 600, padding: 0 \}\}>Edit brand profile \uc0\u8594 </button>\
                  </div>\
                ) : (\
                  <div style=\{\{ textAlign: "center", padding: "20px 0" \}\}>\
                    <div style=\{\{ fontSize: 36, marginBottom: 8 \}\}>\uc0\u55356 \u57256 </div>\
            ...
            <div style=\{card\}>\
              <div style=\{\{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 \}\}>\
                ...
                <button onClick=\{() => setTab("queue")\} style=\{\{ background: "none", border: "none", color: "#6366F1", cursor: "pointer", fontWeight: 600, fontSize: 13 \}\}>View all \uc0\u8594 </button>\
              </div>\
              \{posts.slice(0, 5).map(post => (\
                <div key=\{post.id\} style=\{\{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: "1px solid #F1F5F9" \}\}>\
                  ...
                  <div style=\{\{ flex: 1, minWidth: 0 \}\}>\
                    ...
                    <div style=\{\{ fontSize: 11, color: "#94A3B8", marginTop: 2 \}\}>\uc0\u9200  \{post.date\} \'b7 \{post.type\}</div>\
                  </div>\
                  <Badge s=\{post.status\} />\
                </div>\
              ))\}\
          ...
          <div>\
            ...
            \{brandComplete && (\
              <div style=\{\{ background: "linear-gradient(135deg,#EEF2FF,#F5F3FF)", border: "1px solid #C7D2FE", borderRadius: 10, padding: "10px 16px", marginBottom: 18, display: "flex", alignItems: "center", gap: 10 \}\}>\
                <span style=\{\{ fontSize: 18 \}\}>\{brand.logoEmoji\}</span>\
                <div style=\{\{ fontSize: 13, color: "#4338CA" \}\}>Generating as <strong>\{brand.companyName\}</strong> \'b7 \{brand.industry\} \'b7 Voice: \{brand.brandVoice || "as configured"\}</div>\
              </div>\
            )\}\
            <div style=\{\{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 \}\}>\
              <div style=\{card\}>\
                <h3 style=\{\{ margin: "0 0 16px", fontSize: 14, fontWeight: 700, color: "#0F172A" \}\}>\uc0\u9881 \u65039  Post Settings</h3>\
                  ...
                  <div style=\{\{ display: "flex", flexWrap: "wrap", gap: 7 \}\}>\
                    \{PLATFORMS.map(p => (\
                      <button key=\{p.id\} onClick=\{() => setForm(f => (\{ ...f, pid: p.id \}))\} style=\{\{ display: "flex", alignItems: "center", gap: 5, padding: "6px 10px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 700, background: form.pid === p.id ? p.color : p.bg, color: form.pid === p.id ? "white" : p.color, border: `2px solid $\{form.pid === p.id ? p.color : "transparent"\}` \}\}>\
                        \{p.icon\} \{p.name.split(" ")[0]\} \{platformEnabled[p.id] && "\uc0\u9679 "\}\
                      </button>\
                    ))\}\
                  </div>\
                </Field>\
                ...
                <button onClick=\{generate\} disabled=\{gen\} style=\{\{ width: "100%", padding: "12px", borderRadius: 10, border: "none", cursor: gen ? "wait" : "pointer", background: gen ? "#C7D2FE" : "linear-gradient(135deg,#6366F1,#8B5CF6)", color: "white", fontSize: 14, fontWeight: 700, marginBottom: 14 \}\}>\
                  \{gen ? "\uc0\u9203  Generating..." : "\u10024  Generate Brand-Aligned Content"\}\
                </button>\
              ...
              <div style=\{\{ display: "flex", flexDirection: "column", gap: 14 \}\}>\
                <div style=\{\{ ...card, flex: 1 \}\}>\
                  <h3 style=\{\{ margin: "0 0 12px", fontSize: 14, fontWeight: 700, color: "#0F172A" \}\}>\uc0\u55357 \u56541  Content</h3>\
                  <textarea value=\{form.content\} onChange=\{e => setForm(f => (\{ ...f, content: e.target.value \}))\} placeholder="Brand-aligned AI content appears here\'85" style=\{\{ ...inp, minHeight: 130, resize: "vertical", lineHeight: 1.6 \}\} />\
                ...
                \{form.content && (() => \{\
                  ...
                  return (\
                    <div style=\{\{ background: "white", borderRadius: 12, padding: 16, border: `2px solid $\{p.color\}33` \}\}>\
                      <div style=\{\{ fontSize: 10, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: 1, marginBottom: 9 \}\}>\uc0\u55357 \u56385  Preview</div>\
                      <div style=\{\{ display: "flex", alignItems: "center", gap: 9, marginBottom: 9 \}\}>\
                        <div style=\{\{ width: 32, height: 32, borderRadius: "50%", background: brand.primaryColor || "#6366F1", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 \}\}>\{brand.logoEmoji || "\uc0\u55356 \u57314 "\}</div>\
                        <div><div style=\{\{ fontWeight: 700, fontSize: 13 \}\}>\{brand.companyName || "Your Page"\}</div><div style=\{\{ fontSize: 11, color: "#94A3B8" \}\}>\{form.date || "No date"\} \'b7 \{form.type\}</div></div>\
                ...
                <div style=\{\{ display: "flex", gap: 9 \}\}>\
                  <button onClick=\{() => \{ if (form.content) \{ setPosts(ps => [\{ id: Date.now(), pid: form.pid, content: form.content, topic: form.topic, tone: form.tone, type: form.type, date: "-", status: "draft" \}, ...ps]); setForm(f => (\{ ...f, content: "", tags: "", topic: "" \})); notify("Saved as draft!"); \} \}\} style=\{\{ flex: 1, padding: "12px", borderRadius: 10, border: "1.5px solid #E2E8F0", background: "white", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#374151" \}\}>\uc0\u55357 \u56510  Draft</button>\
                  <button onClick=\{schedule\} disabled=\{saving\} style=\{\{ flex: 2, padding: "12px", borderRadius: 10, border: "none", cursor: saving ? "wait" : "pointer", background: saving ? "#86EFAC" : "linear-gradient(135deg,#10B981,#059669)", color: "white", fontSize: 13, fontWeight: 700 \}\}>\
                    \{saving ? "\uc0\u9203  Scheduling..." : "\u55357 \u56960  Schedule Post"\}\
                  </button>\
                </div>\
              </div>\
            </div>\
            ...
            <div style=\{\{ display: "flex", flexDirection: "column", gap: 10 \}\}>\
              \{posts.length === 0 ? (\
                <div style=\{\{ ...card, textAlign: "center", padding: 60, color: "#94A3B8" \}\}>\
                  <div style=\{\{ fontSize: 44, marginBottom: 10 \}\}>\uc0\u55357 \u56557 </div>\
                ...
                return (\
                  <div key=\{post.id\} style=\{\{ ...card, padding: "15px 18px", display: "flex", gap: 12, alignItems: "flex-start" \}\}>\
                    ...
                    <div style=\{\{ flex: 1, minWidth: 0 \}\}>\
                      ...
                      <div style=\{\{ fontSize: 11, color: "#94A3B8" \}\}>\uc0\u9200  \{post.date\} \'b7 \{post.tone\}</div>\
                    </div>\
                    <div style=\{\{ display: "flex", gap: 6, flexShrink: 0 \}\}>\
                      \{post.status === "scheduled" && <button onClick=\{() => \{ setPosts(ps => ps.map(x => x.id === post.id ? \{ ...x, status: "posted" \} : x)); notify("Marked as posted!"); \}\} style=\{\{ padding: "6px 9px", borderRadius: 7, border: "1px solid #BBF7D0", background: "#F0FDF4", color: "#16A34A", cursor: "pointer", fontSize: 12, fontWeight: 600 \}\}>\uc0\u10003  Posted</button>\}\
                      \{post.status === "draft" && <button onClick=\{() => \{ setPosts(ps => ps.map(x => x.id === post.id ? \{ ...x, status: "scheduled" \} : x)); notify("Scheduled!"); \}\} style=\{\{ padding: "6px 9px", borderRadius: 7, border: "1px solid #C7D2FE", background: "#EEF2FF", color: "#4F46E5", cursor: "pointer", fontSize: 12, fontWeight: 600 \}\}>\uc0\u55357 \u56517  Schedule</button>\}\
                      <button onClick=\{() => \{ setPosts(ps => ps.filter(x => x.id !== post.id)); notify("Deleted."); \}\} style=\{\{ padding: "6px 9px", borderRadius: 7, border: "1px solid #FECACA", background: "#FEF2F2", color: "#DC2626", cursor: "pointer", fontSize: 12, fontWeight: 600 \}\}>\uc0\u10005 </button>\
                    </div>\
                  </div>\
                );\
              \})\}\
          ...
          <div>\
            <div style=\{\{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22 \}\}>\
              ...
              <div style=\{\{ display: "flex", alignItems: "center", gap: 9 \}\}>\
                <button onClick=\{() => setCal(new Date(cal.getFullYear(), cal.getMonth() - 1, 1))\} style=\{\{ padding: "7px 13px", borderRadius: 8, border: "1px solid #E2E8F0", background: "white", cursor: "pointer", fontWeight: 700, fontSize: 15 \}\}>\'8b</button>\
                ...
                <button onClick=\{() => setCal(new Date(cal.getFullYear(), cal.getMonth() + 1, 1))\} style=\{\{ padding: "7px 13px", borderRadius: 8, border: "1px solid #E2E8F0", background: "white", cursor: "pointer", fontWeight: 700, fontSize: 15 \}\}>\'9b</button>\
              </div>\
            </div>\
                ...
                \{Array.from(\{ length: dim(cal.getFullYear(), cal.getMonth()) \}).map((_, i) => \{\
                  ...
                  return (\
                    <div key=\{d\} style=\{\{ minHeight: 90, padding: 7, background: today ? "#EEF2FF" : "white", borderTop: "1px solid #F1F5F9", borderRight: "1px solid #F1F5F9" \}\}>\
                      ...
                      \{dp.slice(0, 3).map(post => \{ const p = PLATFORMS.find(x => x.id === post.pid); return <div key=\{post.id\} style=\{\{ fontSize: 10, background: p?.bg, color: p?.color, borderRadius: 4, padding: "2px 5px", marginBottom: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: 600, borderLeft: `3px solid $\{p?.color\}` \}\}>\{p?.icon\} \{post.content.substring(0, 18)\}\'85</div>; \})\}\
...
\
            <div style=\{\{ display: "flex", gap: 4, marginBottom: 22, background: "white", padding: 4, borderRadius: 12, border: "1px solid #E2E8F0", width: "fit-content" \}\}>\
              \{[\{ id: "brand", l: "\uc0\u55356 \u57256  Brand Profile" \}, \{ id: "apis", l: "\u55357 \u56593  API Keys" \}].map(t => (\
...
\
            \{settingsTab === "brand" && (\
              <div style=\{\{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 \}\}>\
                <div>\
                  <div style=\{card\}>\
                    <h3 style=\{\{ margin: "0 0 18px", fontSize: 15, fontWeight: 700, color: "#0F172A" \}\}>\uc0\u55356 \u57314  Company Identity</h3>\
                ...
                <div style=\{\{ display: "flex", flexDirection: "column", gap: 18 \}\}>\
                  <div style=\{card\}>\
                    <h3 style=\{\{ margin: "0 0 16px", fontSize: 15, fontWeight: 700, color: "#0F172A" \}\}>\uc0\u55356 \u57263  Content Strategy</h3>\
                    <Field label="Target Audience *" hint="Age, interests, pain points, demographics">\
                      <textarea value=\{brand.targetAudience\} onChange=\{e => setBrand(b => (\{ ...b, targetAudience: e.target.value \}))\} placeholder="Women 25\'9640, health-conscious, busy professionals who value natural beauty..." style=\{\{ ...inp, minHeight: 70, resize: "vertical", lineHeight: 1.5 \}\} />\
                    </Field>\
                  ...
                  <button onClick=\{() => notify("\uc0\u9989  Brand profile saved!")\} style=\{\{ padding: "13px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#6366F1,#8B5CF6)", color: "white", fontSize: 14, fontWeight: 700, cursor: "pointer" \}\}>\
                    \uc0\u55357 \u56510  Save Brand Profile\
                  </button>\
                  \{brand.companyName && (\
            ...
            \{settingsTab === "apis" && (\
              <div>\
                <div style=\{\{ background: "linear-gradient(135deg,#FFF7ED,#FFFBEB)", border: "1.5px solid #FED7AA", borderRadius: 12, padding: "14px 18px", marginBottom: 20, display: "flex", gap: 12, alignItems: "flex-start" \}\}>\
                  <span style=\{\{ fontSize: 20, flexShrink: 0 \}\}>\uc0\u9888 \u65039 </span>\
                  <div>\
                    ...
                    <div key=\{p.id\} style=\{\{ ...card, borderLeft: `4px solid $\{platformEnabled[p.id] ? p.color : "#E2E8F0"\}`, transition: "border-color .2s" \}\}>\
                      <div style=\{\{ display: "flex", alignItems: "center", gap: 12, marginBottom: platformEnabled[p.id] ? 16 : 0 \}\}>\
                        ...
                        <div style=\{\{ flex: 1 \}\}>\
                          ...
                          <div style=\{\{ fontSize: 12, color: platformEnabled[p.id] ? "#16A34A" : "#94A3B8" \}\}>\{platformEnabled[p.id] ? "\uc0\u9679  Connected" : "\u9675  Not connected"\}</div>\
                        </div>\
                            ...
                            <Field key=\{field.key\} label=\{field.label\}>\
                              <div style=\{\{ position: "relative" \}\}>\
                                ...
                                <button onClick=\{() => setShowApiKey(s => (\{ ...s, [`$\{p.id\}_$\{field.key\}`]: !s[`$\{p.id\}_$\{field.key\}`] \}))\} style=\{\{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", fontSize: 14, color: "#94A3B8" \}\}>\
                                  \{showApiKey[`$\{p.id\}_$\{field.key\}`] ? "\uc0\u55357 \u56904 " : "\u55357 \u56385 "\}\
                                </button>\
                              </div>\
                            </Field>\
                          ))\}\
                          <button onClick=\{() => \{ const has = p.apiFields.every(f => apiKeys[p.id][f.key].trim()); notify(has ? `\uc0\u9989  $\{p.name\} ready to post!` : `Fill in all $\{p.name\} fields first.`, has ? "ok" : "err"); \}\} style=\{\{ width: "100%", padding: "9px", borderRadius: 8, border: "none", background: p.bg, color: p.color, fontWeight: 700, cursor: "pointer", fontSize: 13 \}\}>\
                            Test Connection\
                          </button>\
                        </div>\
                      )\}\
                ...
                <div style=\{\{ ...card, marginTop: 20 \}\}>\
                  <h3 style=\{\{ margin: "0 0 14px", fontSize: 15, fontWeight: 700, color: "#0F172A" \}\}>\uc0\u55357 \u56523  Auto-Posting Setup Guide</h3>\
